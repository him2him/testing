...
const MENU_IMGG = "https://raw.githubusercontent.com/Mayelprince/url/main/url/1264.jpg"
  ...
  const ownerNumber = ['237682698517','237677224245']
...
if (!fs.existsSync(__dirname + '/sessions/creds.json')) {
    if (!config.SESSION_ID) {
        return console.log('❌ Please add your session to SESSION_ID in config.js or env file!');
    }

    try {
        const base64 = config.SESSION_ID.startsWith("prince_md-")
            ? config.SESSION_ID.replace("prince_md-", "")
            : config.SESSION_ID;
        ...
        console.error("❌ Failed to decode and save session:", err.message);
    }
}
...
const port = process.env.PORT || 9090;
  ...
  console.log('PRINCE-MDXI connected to whatsapp ✅')
...
*╰──────────────●●►*`;
conn.sendMessage(conn.user.id, { image: { url: MENU_IMGG }, caption: up })
